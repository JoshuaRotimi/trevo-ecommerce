        ...
        <tr>
            ...
            <td className={'align-middle'} style={{minWidth: '150px'}}>
                <button className={'btn btn-outline-secondary'}
                        onClick={() => dispatch(decrease(cart, item._id))} disabled={item.quantity === 1}> - </button>
                <span className={'px-3'}>{item.quantity}</span>
                <button className={'btn btn-outline-secondary'}
                        onClick={() => dispatch(increase(cart, item._id))} disabled={item.quantity === item.inStock}> + </button>
            </td>

            <td className={'align-middle'} style={{minWidth: '50px', cursor: "pointer"}}>
               <i className={'far fa-trash-alt text-danger'} aria-hidden={"true"}
                  style={{fontSize: '18px'}} onClick={handleSubmit} />
            </td>
        </tr>
    )
};
